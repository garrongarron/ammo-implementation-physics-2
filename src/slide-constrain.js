...
ammoPromise.then(() => {
    ...
    const ANGLE = Math.PI / 2;//90 degress
    transformA.getBasis().setEulerZYX(0*ANGLE, 3*ANGLE, 0*ANGLE);
    transformA.setOrigin(new Ammo.btVector3(0, 0, 0));



    transformB.getBasis().setEulerZYX(0*ANGLE, -1*ANGLE, -.0*ANGLE);
    // transformB.setRotation(new Ammo.btQuaternion(0, 0, 0, 1));
    transformB.setOrigin(new Ammo.btVector3(0, 3.2, 0));



    // transformA.getBasis().setEulerZYX(0, 0, M_PI_2);
    // transformA.setOrigin(btVector3(0.0f, -1.9f, 0.05));
    // transformB.getBasis().setEulerZYX(0, 0, M_PI_2);
    // transformB.setOrigin(btVector3(0.0, 0.0, -0.1));

    let braso = arm.userData.physicsBody
    let palo = pylon.userData.physicsBody

    let m_forkSlider = new Ammo.btSliderConstraint(braso, palo, transformA, transformB, true)
    // let m_forkSlider = new Ammo.btSliderConstraint( m_forkBody,transformB, true)
    m_forkSlider.setLowerLinLimit(0.1);
    m_forkSlider.setUpperLinLimit(10);
    m_forkSlider.setLowerAngLimit(0.0);
    m_forkSlider.setUpperAngLimit(Math.PI / 2);
    physicsWorld.addConstraint(m_forkSlider, true);
